...
def live_plot(i):
    ...
    if update_plot:
        ...
        for count, test in enumerate(disp_loc):
            ...
            d = dat.load(parameters=[var_to_plot],
                    save_location='%s'%(location))
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        main_text = tk.Label(self, text=('Welcome to the abr_control plotting GUI.'
            ...
            + '\nand will highlight differences between the tests plotted'), font=MED_FONT)
    ...
    def get_selection(self, *args):
        ...
        if len(keys) == 1 and keys[0] is None:
            ...
            if browse_datasets:
                ...
                var_to_plot  = loc[-1][:-1]
                loaded_data = dat.load(parameters=[(loc[-1])[:-1]],
    ...
    def update_param_to_compare(self, *args):
        ...
        if param_to_compare is not None:
            ...
            for ii, test in enumerate(tests_plotted):
                ...
                module_data = dat.load(parameters=dataset_keys, save_location='%s/%s'%(test, param_to_compare))
