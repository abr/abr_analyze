...
class GuiItems:
    ...
    def button(self, frame, text, function, row=1, col=1, f_col=None, b_col=None):
        """
        ...
        butn = tk.Button(frame, text=text, command=function)
    ...
    def entry_box(
        ...
        entry.grid(
            row=row, column=col, columnspan=colspan, rowspan=rowspan, sticky="nsew"
        )
    ...
    def label(
        ...
        if text is not None:
            labl = tk.Label(frame, text=text, font=font, height=height, width=width)
...
class MasterPage(tk.Tk):
    ...
    def __init__(self, db_name, save_location, *args, **kwargs):
        ...
        frame = InterceptScanViewer(
            parent=container, db_name=db_name, save_location=save_location, *args
        )
...
class InterceptScanViewer(tk.Frame):
    ...
    def __init__(self, parent, db_name, save_location, *args):
        ...
        data = self.dat.load(
            parameters=["ideal", "total_intercepts"], save_location=save_location
        )
        ...
        left_bound_entry = self.create.entry_box(
            frame=frame_intercept_val, text_var=left_bound_val, row=0, col=1
        )

        right_bound_entry = self.create.entry_box(
            frame=frame_intercept_val, text_var=right_bound_val, row=0, col=5
        )

        mode_entry = self.create.entry_box(
            frame=frame_intercept_val, text_var=mode_val, row=0, col=3
        )

        # labels for entry boxes
        left_bound_label = self.create.label(
            frame=frame_intercept_val, text="Left", row=1, col=1
        )

        right_bound_label = self.create.label(
            frame=frame_intercept_val, text="Right", row=1, col=5
        )

        mode_label = self.create.label(
            frame=frame_intercept_val, text="Mode", row=1, col=3
        )
...
class LiveFigure:
    ...
    def __init__(self, db_name, save_location):
        ...
        self.ideal = self.dat.load(
            parameters=["ideal"], save_location=self.save_location
        )["ideal"]
        self.test_que = []
        ...
        if update_plot:
            ...
            if clear_plot_val:
                ...
                if data is not None:
                    ...
                    if keep_test_val:
                        current_test = {"label": label, "y": data["y"], "x": data["x"]}
                    ...
                    if save_val:
                        a.figure.savefig(
                            "%s/intercept_scan_%s.png" % (figures_dir, data["title"])
                        )
                        msg = "Figure saved to:" + " %s/intercept_scan_%s.png" % (
                            figures_dir,
                            data["title"],
                        )
                        print(msg)
