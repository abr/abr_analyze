...
class GoogleDoc():
    def __init__(self, title, doc_id=None, id_index_doc_path=None, print_directory=False):
        ...
        if doc_id is None:
            ...
            if doc_id is None:
                ...
                self.doc = self.service.documents() \
                ...
                print('New document id: {0}'.format(
                    doc_id
                ))
    ...
    def insert_image(self, img=None, size=None):
        ...
        img_file = self.drive.files().create(body=file_metadata,
                                            media_body=media,
                                            fields='id').execute()
        img_id = img_file.get('id')
        # print('File ID: %s' % img_id)
        ...
        requests = [{
            'insertInlineImage': {
                ...
                'uri':
                    'https://drive.google.com/uc?export=view&id=%s' % img_id,
                'objectSize': {
                    'height': {
                        'magnitude': size[0]*72,
                        'unit': 'PT'
        ...
        response = self.service.documents().batchUpdate(
            documentId=self.doc_id, body=body).execute()
...
if __name__ == '__main__':
    # create file with the provided title
    doc = googleDoc(title='test_python_generated_file')
    # lines of text to insert
    lines = ['this is the first line', 'this is the second line', 'whoooooo']
    # there formatting, default is NORMAL_TEXT
    styles = ['HEADING_1', 'NORMAL_TEXT', 'HEADING_2']
    # insert text one line at a time, with its formatting
    for ii, line in enumerate(lines):
        doc.add_text(text=line, style_type=styles[ii])
    # insert image with the size specified in inches
    doc.insert_image('Figure_1.png', size=[8,12])
