...
def test_calculate_error(ideal, save_location=save_location):
    ...
    fake_data = dat.load(parameters=[ideal, "ee_xyz"], save_location=save_location)
    manual_error = np.linalg.norm((fake_data["ee_xyz"] - fake_data[ideal]), axis=1)

    traj = TrajectoryError(db_name="test", time_derivative=0, interpolated_samples=None)
