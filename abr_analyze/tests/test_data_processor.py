...
def test_list_to_function():
    results = {}
    test = 'test_list_to_function()'
    results[test] = {}
    print('\n%s----------%s----------%s'%(BLUE, test, ENDC))

    def list_to_function(data, time_intervals, axis, test, label, default_pass,
            results):
        try:
            passed = default_pass
            func = proc.list_to_function(data=data,
    ...
    samples = 44
    results, func = list_to_function(
        data=y,
        time_intervals=x_step,
        axis=0,
        test=test,
        default_pass=True,
        results=results,
        label='Function from 1D array')
    ax[0].plot(x, y, 'o', label='raw')
    x_new = np.linspace(x_step[0], x[-1], samples)
    ax[0].plot(x_new, func(x_new), label='interpolated')
    ax[0].set_title('Function from 1D array')
    ax[0].legend()
    ...
    results, func = list_to_function(
        ...
        default_pass=True,
        results=results,
        label='Function from 2D array, axis=1')
    ax[1].plot(x, y[0], 'o', label='raw sin')
    ax[1].plot(x, y[1], 'o', label='raw cos')
    x_new = np.linspace(x_step[0], x[-1], samples)
    ax[1].plot(x_new, func[0](x_new), label='interpolated sin')
    ax[1].plot(x_new, func[1](x_new), label='interpolated cos')
    ax[1].set_title('Function from 2D array, axis=1')
    ax[1].legend()

    # passing in 2D list, interpolated on other axis
    x = np.linspace(0, 6.28, 100)
    ...
    results, func = list_to_function(
        ...
        default_pass=True,
        results=results,
        label='Function from 2D array, axis=0')
    ax[2].plot(x, y[:, 0], 'o', label='raw sin')
    ax[2].plot(x, y[:, 1], 'o', label='raw cos')
    x_new = np.linspace(x_step[0], x[-1], samples)
    ax[2].plot(x_new, func[0](x_new), label='interpolated sin')
    ax[2].plot(x_new, func[1](x_new), label='interpolated cos')
    ax[2].set_title('Function from 2D array, axis=0')
    ax[2].legend()
...
def test_interpolate_data():
    results = {}
    test = 'test_interpolate_data()'
    results[test] = {}
    print('\n%s----------%s----------%s'%(BLUE, test, ENDC))

    def interpolate_data(data, time_intervals, interpolated_samples, axis,
            default_pass, results, label):
        try:
            passed = default_pass
...
dat = DataHandler(db)
proc = DataProcessor()
#generate_random_traj()
test_get_mean_and_ci()
test_list_to_function()
test_interpolate_data()
test_scale_data()
test_filter_data()
test_load_and_process()
test_calc_cartesian_points()
test_two_norm_error()
