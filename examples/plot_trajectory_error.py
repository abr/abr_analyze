...
for jj, td in enumerate(time_derivatives):
    ...
    traj = TrajectoryError(
        db_name=db_name, time_derivative=td, interpolated_samples=interpolated_samples
    )
    for ii, test in enumerate(test_list):
        print("Processing test %i/%i: %s" % (ii + 1, len(test_list), test))
        traj.statistical_error(
            save_location="%s" % (test), sessions=sessions, runs=runs, regen=True
        )
    print("Plotting...")
    c = ["b", "r"]
    for ii, test in enumerate(test_list):
